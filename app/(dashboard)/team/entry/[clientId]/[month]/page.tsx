...
// ── Platform config ───────────────────────────────────────────────────────────
const PLATFORM_CONFIG: Record<Tab, { gradient: string; lightBg: string; ring: string; textColor: string; icon: React.ReactNode }> = {
  Instagram: {
    gradient: "from-pink-500 via-rose-500 to-purple-600", lightBg: "bg-pink-50", ring: "ring-pink-300", textColor: "text-pink-700",
  ...
  Facebook: {
    gradient: "from-blue-600 to-blue-500", lightBg: "bg-blue-50", ring: "ring-blue-300", textColor: "text-blue-700",
  ...
  TikTok: {
    gradient: "from-gray-900 to-gray-700", lightBg: "bg-gray-100", ring: "ring-gray-400", textColor: "text-gray-700",
  ...
  YouTube: {
    gradient: "from-red-600 to-red-500", lightBg: "bg-red-50", ring: "ring-red-300", textColor: "text-red-700",
  ...
  GMB: {
    gradient: "from-orange-500 to-amber-400", lightBg: "bg-orange-50", ring: "ring-orange-300", textColor: "text-orange-700",
    icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C7.802 0 4 3.403 4 7.602C4 11.8 7.469 16.812 12 24C16.531 16.812 20 11.8 20 7.602C20 3.403 16.199 0 12 0ZM12 11C10.343 11 9 9.657 9 8C9 6.343 10.343 5 12 5C13.657 5 15 6.343 15 8C15 9.657 13.657 11 12 11Z"/></svg>,
  },
  Website: {
    gradient: "from-teal-600 to-cyan-500", lightBg: "bg-teal-50", ring: "ring-teal-300", textColor: "text-teal-700",
  ...
  Email: {
    gradient: "from-violet-600 to-purple-500", lightBg: "bg-violet-50", ring: "ring-violet-300", textColor: "text-violet-700",
...
function prevMonthPeriod(period: string): string {
  const [y, m] = period.split("-").map(Number);
  const d = new Date(y, m - 2, 1);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
}

function fmtPrev(val: string | undefined): string {
  if (!val || val === "") return "—";
...
export default function TeamEntryPage() {
  ...
  // Previous month reference data (keyed by Tab)
  const [prevData, setPrevData] = useState<Record<Tab, FormValues>>({
    Instagram: {}, Facebook: {}, TikTok: {}, YouTube: {}, GMB: {}, Website: {}, Email: {},
  });
  const [hasPrevData, setHasPrevData] = useState(false);

  const stateMap: Record<Tab, [FormValues, React.Dispatch<React.SetStateAction<FormValues>>]> = {
  ...
  // Helper to mark dirty whenever a form value changes
  const makeOnChange = useCallback(
    (setter: React.Dispatch<React.SetStateAction<FormValues>>, key: string) =>
      (val: string) => {
        setter((f) => ({ ...f, [key]: val }));
        setIsDirty(true);
      },
  ...
  // ── Load existing data + previous month reference ───────────────────────────
  useEffect(() => {
    const init = async () => {
      // Client name
      ...
      if (reportsRes.ok) {
        const reports: { id: string; period: string; status: string; notes: string | null; readyForReview: boolean }[] = await reportsRes.json();
        const existing = reports.find((r) => r.period === month);
        if (existing) {
          setReportId(existing.id);
          setReportStatus(existing.status);
          setReadyForReview(existing.readyForReview ?? false);
      ...
      // Previous month reference data
      const prev = prevMonthPeriod(month);
      const prevReports = await fetch(`/api/reports?clientId=${clientId}&period=${prev}`);
      if (prevReports.ok) {
        const pReports: { id: string; period: string }[] = await prevReports.json();
        const prevReport = pReports.find((r) => r.period === prev);
        if (prevReport) {
          const pData = await fetch(`/api/reports/${prevReport.id}/data`);
          if (pData.ok) {
            const d = await pData.json();
            const toStr = (obj: Record<string, unknown> | null | undefined): FormValues =>
              obj ? Object.fromEntries(
                Object.entries(obj).filter(([, v]) => v != null).map(([k, v]) => [k, String(v)])
              ) : {};
            const pd: Record<Tab, FormValues> = {
              Instagram: toStr(d.socialMedia?.instagram),
              Facebook:  toStr(d.socialMedia?.facebook),
              TikTok:    toStr(d.socialMedia?.tiktok),
              YouTube:   toStr(d.socialMedia?.youtube),
              GMB:       toStr(d.gmbData),
              Website:   toStr(d.websiteData),
              Email:     toStr(d.emailMarketing),
            };
            setPrevData(pd);
            setHasPrevData(Object.values(pd).some((v) => Object.keys(v).length > 0));
          }
        }
      }
  ...
  const copyFromPrev = () => {
    ...
    setFacebook({ ...prevData.Facebook });
    setTiktok({ ...prevData.TikTok });
    setYoutube({ ...prevData.YouTube });
    setGmb({ ...prevData.GMB });
    setWebsite({ ...prevData.Website });
    setEmail({ ...prevData.Email });
    setIsDirty(true);
    toastSuccess("Copied!", `Previous month data filled in. Review and save when ready.`);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsDirty(false);
      toastSuccess("Saved!", "All platform data and notes saved successfully.");
    } catch (err) {
  ...
  const handleSubmitForReview = async () => {
    ...
    try {
      ...
      // Save data first
      await fetch(`/api/reports/${id}/data`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          instagram: parseFormToNumbers(instagram),
      ...
      const res = await fetch(`/api/reports/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ notes, readyForReview: true }),
      });
      if (!res.ok) { const d = await res.json(); throw new Error(d.error ?? "Failed"); }

      setReadyForReview(true);
      setIsDirty(false);
      toastSuccess("Submitted for review!", "Admin will be notified to review and publish this report.");
    } catch (err) {
  ...
  const prevPeriodLabel = periodLabel(prevMonthPeriod(month));
    ...
    <div className="space-y-5 page-content">
      ...
      {isDirty && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <p className="text-sm text-amber-800 font-medium">You have unsaved changes — click <span className="font-bold">Save All Data</span> before leaving.</p>
        </div>
      )}
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-blue-700 rounded-2xl p-5 shadow-lg shadow-indigo-200/40">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <Link
              href="/team"
              className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/15 hover:bg-white/25 border border-white/20 text-white transition-colors flex-shrink-0"
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border ${
                  ...
                  <span className={`w-1.5 h-1.5 rounded-full ${reportStatus === "PUBLISHED" ? "bg-emerald-400" : readyForReview ? "bg-purple-400" : "bg-amber-400"}`} />
                  {reportStatus === "PUBLISHED" ? "Published" : readyForReview ? "In Review" : "Draft"}
                </span>
              </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="bg-white/15 border border-white/20 rounded-xl px-4 py-2 flex items-center gap-2.5">
              <div className="flex gap-1">
                {TABS.map((t) => (
                  <div key={t} className={`w-2 h-2 rounded-full transition-colors ${hasData(stateMap[t][0]) ? "bg-emerald-400" : "bg-white/25"}`} title={t} />
                ))}
              </div>
              <span className="text-white text-xs font-semibold">{filledCount}/{TABS.length}</span>
            </div>
            {/* Copy from prev */}
            {hasPrevData && (
              <button
                onClick={copyFromPrev}
                className="inline-flex items-center gap-1.5 bg-white/15 hover:bg-white/25 border border-white/20 text-white text-xs font-semibold px-3 py-2 rounded-xl transition-colors"
                title={`Copy all data from ${prevPeriodLabel}`}
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                </svg>
                Copy from {prevPeriodLabel}
              </button>
            )}
            <Link
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`relative flex-shrink-0 flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all duration-150 ${active ? "bg-gray-900 text-white shadow-sm" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}
              >
                ...
                {tab}
                {filled && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" />}
              </button>
            );
          })}
        </div>
        ...
        <div className="hidden lg:block bg-white rounded-2xl shadow-sm border border-gray-100 p-3 h-fit">
          ...
          <div className="space-y-1">
            {TABS.map((tab) => {
              ...
              return (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 text-left group ${active ? "bg-gray-900 text-white shadow-sm" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
                >
            ...
            <div className="ml-auto flex items-center gap-2">
              {hasPrevData && (
                <span className="text-white/60 text-xs font-medium hidden sm:block">
                  Prev: {prevPeriodLabel}
                </span>
              )}
              {hasData(values) && (
                <span className="inline-flex items-center gap-1.5 bg-white/20 border border-white/25 text-white text-xs font-semibold px-3 py-1.5 rounded-full">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {FIELDS[activeTab].map(({ key, label, hint, isFloat }) => {
                ...
                const prevVal    = prevData[activeTab]?.[key];
                const filled     = currentVal !== "";
                return (
                  <div key={key} className="group">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      {prevVal !== undefined && prevVal !== "" && (
                        <span className="text-[10px] font-medium text-gray-400 bg-gray-50 border border-gray-100 px-1.5 py-0.5 rounded-md">
                          Prev: {fmtPrev(prevVal)}
                        </span>
                      )}
                    </div>
                    {hint && <p className="text-xs text-gray-400 mb-2 leading-tight">{hint}</p>}
                    <div className="relative">
                      <input
                        ...
                        min="0"
                        value={currentVal}
                        onChange={(e) => makeOnChange(stateMap[activeTab][1], key)(e.target.value)}
                        placeholder={prevVal ? fmtPrev(prevVal) : "0"}
                        className={`w-full border rounded-xl px-4 py-3 text-sm font-medium text-gray-900 bg-gray-50 focus:outline-none focus:ring-2 focus:border-transparent transition-all placeholder:text-gray-300 ${
                          filled ? "border-emerald-200 focus:ring-emerald-400 bg-emerald-50/30" : "border-gray-200 focus:ring-indigo-400 hover:border-gray-300"
                        }`}
                      />
                      {filled && (
            ...
            <div className="flex items-center justify-between mt-8 pt-5 border-t border-gray-100">
              <button
                onClick={() => { const idx = TABS.indexOf(activeTab); if (idx > 0) setActiveTab(TABS[idx - 1]); }}
              ...
              <span className="text-xs text-gray-400 font-medium">{TABS.indexOf(activeTab) + 1} of {TABS.length}</span>
              <button
                onClick={() => { const idx = TABS.indexOf(activeTab); if (idx < TABS.length - 1) setActiveTab(TABS[idx + 1]); }}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <label className="text-sm font-bold text-gray-700">Note for Client</label>
          <span className="text-[10px] text-gray-400 bg-gray-50 border border-gray-100 px-2 py-0.5 rounded-full">Visible to client on their report</span>
        </div>
        <textarea
          value={notes}
          onChange={(e) => { setNotes(e.target.value); setIsDirty(true); }}
          placeholder="Add a summary, highlight wins, or explain any data for this month… (optional)"
          rows={3}
          className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent resize-none hover:border-gray-300 transition-colors placeholder:text-gray-300"
        />
      </div>

      {/* ── Save bar ────────────────────────────────────────────────────────── */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-4 flex items-center gap-3 flex-wrap">
        {/* Save */}
        <button
          onClick={handleSave}
          disabled={saving}
        ...
        >
          {saving ? (
            ...
            <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>Save All Data</>
          )}
        </button>
        ...
        {reportStatus !== "PUBLISHED" && (
          <button
            ...
            className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all border ${
              readyForReview
                ? "bg-purple-50 text-purple-600 border-purple-200 cursor-default"
                : "bg-purple-600 text-white border-transparent hover:bg-purple-700 shadow-sm hover:-translate-y-0.5"
            } disabled:opacity-70`}
          >
            {submitting ? (
              ...
              <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>In Review</>
            ) : (
              <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>Submit for Review</>
            )}
          </button>
        )}
        ...
        <div className="ml-auto text-xs text-gray-400 hidden sm:block">
          {filledCount === TABS.length ? "✓ All platforms filled" : `${TABS.length - filledCount} platform${TABS.length - filledCount !== 1 ? "s" : ""} still empty`}
        </div>
      </div>
    </div>
  );
